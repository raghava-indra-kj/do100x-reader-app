...
import { Workflow, ZoomIn, ZoomOut, Maximize, X } from "lucide-react";
import type { MdViewColors } from "../../types/theme";

export interface D2FullscreenModalProps {
  svg: string;
...
export function D2FullscreenModal({ svg, colors, onClose, title = "D2 Diagram Fullscreen" }: D2FullscreenModalProps) {
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if ((e.target as HTMLElement).closest(".md-d2-fullscreen-header")) return;
    e.preventDefault();
  ...
  return createPortal(
    <div className="md-d2-fullscreen-modal" style={bgStyle}>
      {/* Header Toolbar */}
      <header className="md-d2-fullscreen-header">
        <div className="md-d2-fullscreen-title">
          <Workflow className="md-d2-icon" size={18} />
          <span>{title}</span>
        </div>

        <div className="md-d2-fullscreen-controls">
          <button
            type="button"
          ...
          <div className="md-d2-control-divider" />
      ...
      <div
        ref={canvasRef}
        className={`md-d2-fullscreen-canvas ${isDragging ? "md-d2-canvas--dragging" : ""}`}
      ...
      >
        <div
          ref={contentRef}
          className="md-d2-fullscreen-content"
          style={{
