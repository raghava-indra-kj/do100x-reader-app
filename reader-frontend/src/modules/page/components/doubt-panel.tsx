...
import { MessageSquare, X, Maximize2, Minimize2, Trash2, FilePlus, Check } from 'lucide-react';
...
export const PageDoubtPanel = observer(function PageDoubtPanel() {
    ...
    const activeEntry = doubtStore.activeEntry;
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="flex items-center justify-between shrink-0 px-3 pt-3 pb-2 border-b border-[var(--color-border-subtle)]">
                <span className="text-xs font-semibold text-[var(--color-text-subtle)] uppercase tracking-wider flex items-center gap-1.5">
                    <MessageSquare size={12} className="text-[var(--color-brand)]" />
                    <span>AI Doubt</span>
                </span>
                <div className="flex items-center gap-2">
                    {activeEntry && activeEntry.responseMarkdown && !activeEntry.isLoading && (
                        <button
                    ...
                    {doubtStore.history.length > 0 && (
                        <button
                            onClick={() => doubtStore.clearHistory()}
                    ...
                    <button
                        onClick={() => doubtStore.toggleExpand()}
                        className="p-1 text-[var(--color-text-muted)] hover:text-[var(--color-text-strong)] transition-colors cursor-pointer"
                        title={doubtStore.isExpanded ? "Restore" : "Expand to full width"}
                    >
                        {doubtStore.isExpanded ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
                    </button>
                    <button
                        onClick={() => doubtStore.close()}
                        className="p-1 text-[var(--color-text-muted)] hover:text-[var(--color-text-strong)] transition-colors cursor-pointer"
                        title="Close"
                    >
                        <X size={14} />
            ...
            {doubtStore.history.length > 0 && (
                <div className="flex items-center gap-1.5 overflow-x-auto px-3 py-2 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-soft)] shrink-0 scrollbar-none">
                    {doubtStore.history.map((entry) => (
                        <div
                            key={entry.id}
                            onClick={() => doubtStore.setActiveEntry(entry.id)}
                            className={`flex items-center gap-1.5 shrink-0 px-2.5 py-1 text-[10px] font-medium rounded-full cursor-pointer transition-colors border select-none ${
                                doubtStore.activeEntryId === entry.id
                        ...
                        >
                            ...
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    doubtStore.removeEntry(entry.id);
                                }}
                ...
                {activeEntry ? (
                    activeEntry.isLoading ? (
                        <div className="flex flex-col items-center justify-center h-48 gap-3">
                            <Loader size={24} />
                            <span className="text-xs text-[var(--color-text-subtle)]">Querying AI for doubt...</span>
                        </div>
                    ) : activeEntry.error ? (
                        ...
                        <div className="space-y-4">
                            <div>
                                <span className="text-[10px] text-[var(--color-text-subtle)] font-semibold uppercase tracking-wider block mb-1">Doubt:</span>
                                <p className="text-xs italic text-[var(--color-text-strong)] bg-[var(--color-surface-soft)] border-l-2 border-[var(--color-brand)] px-2.5 py-1.5 rounded">
                                    "{activeEntry.searchTerm}"
                                </p>
                            </div>
                            <div className="border-t border-[var(--color-border-subtle)] pt-4">
                                <span className="text-[10px] text-[var(--color-text-subtle)] font-semibold uppercase tracking-wider block mb-2">Answer:</span>
                                <MarkdownRenderer
                            ...
                            <div className="border-t border-[var(--color-border-subtle)] pt-4 space-y-2 shrink-0">
                                <span className="text-[10px] text-[var(--color-text-subtle)] font-semibold uppercase tracking-wider block">Re-ask or Rephrase Doubt:</span>
                                <div className="flex gap-2 items-stretch">
                                    <textarea
                                        value={rephraseText}
                                        onChange={(e) => setRephraseText(e.target.value)}
                                        placeholder="Rephrase your question or ask a follow-up..."
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            ...
                                            doubtStore.reaskDoubt(activeEntry.id, rephraseText);
                                        }}
                                    >
                                        Re-ask
                                    </Button>
                    ...
                    <div className="text-center text-xs text-[var(--color-text-subtle)] pt-12">
                        Select text on the page, write a question in "Ask Doubt", and click "Ask AI" to explain.
                    </div>
                )}
            </div>
        </div>
