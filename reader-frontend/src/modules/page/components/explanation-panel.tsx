...
import { Compass, X, Maximize2, Minimize2, Trash2 } from 'lucide-react';
...
export const PageExplanationPanel = observer(function PageExplanationPanel() {
    ...
    const explanationStore = store.explanationStore;
    ...
    const activeEntry = explanationStore.activeEntry;
    const [rephraseText, setRephraseText] = useState('');

    useEffect(() => {
        if (activeEntry) {
            setRephraseText(activeEntry.selectedText);
        } else {
            setRephraseText('');
        }
    }, [activeEntry?.id]);
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="flex items-center justify-between shrink-0 px-3 pt-3 pb-2 border-b border-[var(--color-border-subtle)]">
                <span className="text-xs font-semibold text-[var(--color-text-subtle)] uppercase tracking-wider flex items-center gap-1.5">
                    <Compass size={12} className="text-[var(--color-brand)]" />
                    <span>AI Explanation</span>
                </span>
                <div className="flex items-center gap-2">
                    {explanationStore.history.length > 0 && (
                        <button
                            onClick={() => explanationStore.clearHistory()}
                    ...
                    <button
                        onClick={() => explanationStore.toggleExpand()}
                        className="p-1 text-[var(--color-text-muted)] hover:text-[var(--color-text-strong)] transition-colors cursor-pointer"
                        title={explanationStore.isExpanded ? "Restore" : "Expand to full width"}
                    >
                        {explanationStore.isExpanded ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
                    </button>
                    <button
                        onClick={() => explanationStore.close()}
                        className="p-1 text-[var(--color-text-muted)] hover:text-[var(--color-text-strong)] transition-colors cursor-pointer"
                        title="Close"
                    >
                        <X size={14} />
            ...
            {explanationStore.history.length > 0 && (
                <div className="flex items-center gap-1.5 overflow-x-auto px-3 py-2 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-soft)] shrink-0 scrollbar-none">
                    {explanationStore.history.map((entry) => (
                        <div
                            key={entry.id}
                            onClick={() => explanationStore.setActiveEntry(entry.id)}
                            className={`flex items-center gap-1.5 shrink-0 px-2.5 py-1 text-[10px] font-medium rounded-full cursor-pointer transition-colors border select-none ${
                                explanationStore.activeEntryId === entry.id
                        ...
                        >
                            <span className="truncate max-w-[80px]">{entry.selectedText}</span>
                            {entry.isLoading && (
                            ...
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    explanationStore.removeEntry(entry.id);
                                }}
                ...
                {activeEntry ? (
                    activeEntry.isLoading ? (
                        <div className="flex flex-col items-center justify-center h-48 gap-3">
                            <Loader size={24} />
                            <span className="text-xs text-[var(--color-text-subtle)]">Generating AI explanation...</span>
                        </div>
                    ) : activeEntry.error ? (
                        <div className="rounded-lg bg-[var(--color-surface-soft)] border border-[var(--color-border-subtle)] p-3 text-xs text-[var(--color-text-body)]">
                            <p className="font-semibold text-[var(--color-text-strong)]">Explanation Issue</p>
                            <p className="mt-1 leading-relaxed text-[var(--color-text-muted)]">{activeEntry.error}</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            <div>
                                <span className="text-[10px] text-[var(--color-text-subtle)] font-semibold uppercase tracking-wider block mb-1">Passage:</span>
                                <p className="text-xs italic text-[var(--color-text-strong)] bg-[var(--color-surface-soft)] border-l-2 border-[var(--color-brand)] px-2.5 py-1.5 rounded leading-relaxed">
                                    "{activeEntry.selectedText}"
                                </p>
                            </div>
                            <div className="border-t border-[var(--color-border-subtle)] pt-4">
                                <span className="text-[10px] text-[var(--color-text-subtle)] font-semibold uppercase tracking-wider block mb-2">AI Explanation:</span>
                                <MarkdownRenderer
                            ...
                            <div className="border-t border-[var(--color-border-subtle)] pt-4 space-y-2 shrink-0">
                                <span className="text-[10px] text-[var(--color-text-subtle)] font-semibold uppercase tracking-wider block">Rephrase Passage / Instructions:</span>
                                <div className="flex gap-2 items-stretch">
                                    <textarea
                                        value={rephraseText}
                                        onChange={(e) => setRephraseText(e.target.value)}
                                        placeholder="Modify passage or add custom instructions..."
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            ...
                                            explanationStore.reaskExplanation(activeEntry.id, rephraseText);
                                        }}
                                    >
                                        Re-ask
                                    </Button>
                    ...
                    <div className="text-center text-xs text-[var(--color-text-subtle)] pt-12">
                        Select text on the page and click "AI Explain" to get an explanation.
                    </div>
                )}
            </div>
        </div>
